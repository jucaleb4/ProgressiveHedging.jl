...
if nworkers() < WORKERS
    ...
    for w in workers()
        ...
        # @spawnat(w, Pkg.instantiate())
        # @spawnat(w, Pkg.gc())
        # @spawnat(w, ENV["XPRESSDIR"]="/nopt/nrel/apps/xpressmp/8.13.0")
    end
end
...
# for w in workers()
#    @spawnat(w, Pkg.activate("../j16"))
# end
# @everywhere ENV["JULIA_DEPOT_PATH"]="/projects/pvb/cju/.julia"
# @everywhere ENV["PATH"]="/home/cju/julia-1.7.2/bin:$PATH"
# @everywhere Pkg.precompile()
# @everywhere Pkg.gc()
# @everywhere ENV["XPRESSDIR"]="/nopt/nrel/apps/xpressmp/8.13.0"
# @everywhere Pkg.build("Xpress")

# https://discourse.julialang.org/t/failed-to-precompile-error-on-worker-processes/40638
# @everywhere using Ipopt
@everywhere using Ipopt
@everywhere using JuMP, Xpress, PowerSimulations, PowerSystems, PowerSystemCaseBuilder, InfrastructureSystems, Dates, TimeSeries
# using JuMP, PowerSimulations, PowerSystems, PowerSystemCaseBuilder, TimeSeries
