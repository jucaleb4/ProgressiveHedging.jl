...
@everywhere function create_model(
    ...
    @constraint(ed, [t=1:T], sum(g[:, t] + h[:, t]) + w[t] == d[t,1])
    
    # Define the constraint on the wind power injection and power balance 
    # if scenario_id == PH.scid(0)
    tid = PH.value(scenario_id)
    if 0 <= tid <= 3
        tid += 1

        # Define the objective function
        @objective(ed, Min, sum(c_g[i]*g[i, tid] for i in 1:n) + sum(c_w *w[tid]))
...
@everywhere function create_model2(
        scenario_id::PH.ScenarioID,
        key_word_arg::String="Default key word argument", kwargs...
        )
    ed = Model(Xpress.Optimizer)
    ...
    # Define decision variables    
    stage1 = Vector{JuMP.VariableRef}()
    vref = @variable(ed, b[t = 1:T+1]) # total budget remaining
    append!(stage1, vref)
    ...
    if 0 <= tid <= 3
        ...
        # Define the constraint on the maximum and minimum power output of each generator
        @constraint(ed, [t = tid:tid+1], 0 <= b[t] <= h_budget) # total budget remaining

        @constraint(ed, [i = 1:n], g_min[i] <= g[i, tid] <= g_max[i]) 

        @constraint(ed, [i = 1:n], h_min[i] <= h[i, tid] <= h_max[i])

        # Define the constraint on the wind power injection
        @constraint(ed, w[tid] <= w_f[tid])

        # Budget constraint
        @constraint(ed, sum(h[i, tid] for i in 1:n) <= h_b[tid])
        ...
        @constraint(ed, b[tid+1] - b[tid] + h_b[tid] == 0)

        @constraint(ed, sum(g[:, tid] + h[:, tid]) + w[tid] == d[tid,2])

        # Define the objective function
        @objective(ed, Min, sum(c_g[i]*g[i, tid] for i in 1:n) + c_w *w[tid])

    else
        # TODO: print the wrong scenario ID
        println(">> Scenario id:", tid)
        @error "Incorrect scenario id"
    end
